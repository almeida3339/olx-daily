...
# Rodamos SEMPRE sob 'Continue': comandos git/node são nativos e emitem stderr
...
# explicitamente. Sem dança de $ErrorActionPreference (causava erro de null).
...
Write-Host "[2/4] Coletando Mercado Livre (invisível; pode levar ~15-20 min)..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) { Fail "Geração do dashboard falhou (exit $LASTEXITCODE)." }
...
for ($attempt = 1; $attempt -le 4; $attempt++) {
  ...
  git fetch origin
  git -c core.editor=true rebase -X theirs origin/main
  if ($LASTEXITCODE -ne 0) { git rebase --abort 2>$null; Fail "Rebase pré-push falhou; estado limpo." }
  node (Join-Path $PSScriptRoot "generate-dashboard.mjs")
  git add index.html
  if (-not (git diff --staged --quiet)) { git commit --amend --no-edit | Out-Null }
}
Fail "git push falhou após 4 tentativas."
